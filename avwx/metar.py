...
def parse_na(report: str) -> (MetarData, Units):
    ...
    wxdata, wxresp['wind_direction'], wxresp['wind_speed'], \
        wxresp['wind_gust'], wxresp['wind_variable_direction'] = _core.get_wind(wxdata, units)
    wxdata, wxresp['altimeter'] = _core.get_altimeter(wxdata, units, 'NA')
    wxdata, wxresp['visibility'] = _core.get_visibility(wxdata, units)
    wxresp['other'], wxresp['temperature'], wxresp['dewpoint'] = _core.get_temp_and_dew(wxdata)
    condition = _core.get_flight_rules(wxresp['visibility'], _core.get_ceiling(wxresp['clouds']))
