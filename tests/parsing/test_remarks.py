...
def test_temp_dew_decimal(codes: list[str], temperature: float, dewpoint: float) -> None:
    """Test extracting temperature and dewpoint decimal values."""
...
def test_temp_minmax(codes: list[str], maximum: float, minimum: float) -> None:
    """Test extracting 24-hour min max temperatures."""
...
def test_precip_snow(codes: list[str], precipitation: float | None, snowlevel: int | None) -> None:
    """Test extracting hourly precipitation and snow accumulation."""
...
def test_sea_level_pressure(codes: list[str], pressure: float) -> None:
    """Test extracting the inferred sea level pressure in mb."""
    codes, sea = remarks.sea_level_pressure(codes)
    assert codes == ["123"]
    assert_value(sea, pressure)


def test_no_sea_level_pressure() -> None:
    codes = ["123", "SLPNO"]
...
def test_parse() -> None:
    """Test full remarks parsing."""
    assert remarks.parse("") is None
    rmk = "RMK 10123 21234 51234 61234 74321 98321 " "T01231234 403451567 P1234 4/123 SLP998 " "ACFT MSHP TSB20 AO2 $"
