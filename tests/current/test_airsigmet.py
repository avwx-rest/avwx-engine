...
_COORD_REPORTS = [airsigmet.AirSigmet.from_report(_PRE + r) for r in _COORD_LOCS]
COORD_REPORTS: list[airsigmet.AirSigmet] = [i for i in _COORD_REPORTS if i]
...
def test_parse_prep(source: str, target: str) -> None:
...
def test_bulletin(repr: str, type: str, country: str, number: int) -> None:
...
def test_header(wx: str, bulletin: str, issuer: str, time: str, correction: str | None) -> None:
...
def test_first_index(index: int, targets: tuple[str]) -> None:
...
def test_region(wx: str, region: str, extra: str) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            core.make_timestamp("0900", time_only=True),
            core.make_timestamp("1500Z", time_only=True),
            "THRU 15Z <break>",
        ),
        (
            "VA CLD OBS AT 0110Z SFC/FL160",
            core.make_timestamp("0110Z", time_only=True),
            None,
            "VA CLD SFC/FL160",
        ),
    ],
)
def test_time(wx: str, start: structs.Timestamp, end: structs.Timestamp | None, extra: str) -> None:
...
def test_coord_value(coord: str, value: float) -> None:
...
def test_position(wx: str, coord: Coord | None, extra: str) -> None:
...
def test_bounds_from_latterals(report: str, bounds: list[str], start: int, extra: str) -> None:
...
def test_coords_from_text(report: str, coords: tuple[tuple], start: int, extra: str) -> None:
...
def test_coords_from_navaids(report: str, coords: tuple[tuple], start: int, extra: str) -> None:
...
def test_bounds(wx: str, coords: tuple[tuple], bounds: list[str], extra: str) -> None:
...
def test_weather_type(wx: str, code: str | None, name: str | None, extra: str) -> None:
    """Test extracting weather type code from report."""
...
def test_intensity(wx: str, code: str | None, name: str | None, extra: str) -> None:
    """Test extracting intensity code from report."""
...
def test_sanitize(report: str, clean: str) -> None:
...
@pytest.mark.parametrize("ref", json.load(AIRSIG_PATH.open(), object_hook=datetime_parser))
def test_airsigmet_ete(ref: dict) -> None:
    """Perform an end-to-end test of reports in the AIRSIGMET JSON file."""
...
def test_manager_contains(lat: int, lon: int, count: int) -> None:
...
def test_along(coords: tuple, count: int) -> None:
