...
def test_root(root: str, station: Optional[str], report_type: Optional[str]):
...
def test_location(
    loc: str, station: Optional[str], direction: Optional[int], distance: Optional[int]
):
...
def test_altitude(alt: str, num: int):
...
def test_aircraft(code: str, name: str):
...
def test_cloud_tops(cloud: str, data: list):
...
def test_turbulence(
    turb: str, severity: str, floor: Optional[int], ceiling: Optional[int]
):
...
def test_wx(
    text: str, wx: List[Tuple[str, str]], vis: Optional[int], remain: List[str]
):
...
def test_pirep_ete(ref: dict, icao: str, issued: datetime):
