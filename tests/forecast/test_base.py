...
def test_trim_lines(target: int, length: int) -> None:
    """Test trimming line length based on the length of a target line."""
...
def test_split_line(size: int, prefix: int, expect: list[str]) -> None:
    """Test splitting and stripping elements on a non-delineated line."""
    text = " 1  2  3  4  5  6  7  8  9  0 "
    assert base._split_line(text, size, prefix) == expect


def test_split_mav_line() -> None:
...
def test_timestamp(year: int, month: int, day: int, hour: int, minute: int, line: str) -> None:
    """Test Timestamp inference on the first line of the report."""
    time = base._timestamp(line)
    assert isinstance(time, structs.Timestamp)
    assert isinstance(time.repr, str)
    assert time.dt is not None
...
@pytest.mark.parametrize(
    ...
    [
        (
            [(18, 3), (2, 6)],
            base._split_line("HR   06 09 12 15 18 21 00 03 06 09 12 15 18 21 00 03 06 09 12 18 00 "),
        ),
        (
            [(14, 12)],
            base._split_line(
...
def test_init_parse(report: str, time: datetime, line_length: int) -> None:
    """Test first pass data creation and line splits."""
...
class ForecastBase:
    report: type[base.Forecast]

    def test_forecast_ete(self, ref: dict, icao: str, issued: datetime) -> None:
        """Perform an end-to-end test of all report JSON files."""
